 ...
 */    var MEDIA_PREFERS_REDUCED_MOTION = "(prefers-reduced-motion: reduce)";
    ...
    document.addEventListener("DOMContentLoaded", (function() {
        var speakersSliderEls = document.querySelectorAll(".speakers-home__slider, .splide--def");
        speakersSliderEls.forEach((function(sliderEl) {
            var speakersSlider = new Splide(sliderEl, {
                perPage: 4,
                arrows: true,
                pagination: false,
                perMove: 1,
    ...
    document.addEventListener("click", (e => {
        ...
        if (e.target.closest(".item-speakers")) {
            const clickedEl = e.target.closest(".item-speakers");
            document.querySelectorAll(".item-speakers")?.forEach((item => {
                item !== clickedEl ? item.classList.remove("_active") : null;
            }));
            clickedEl.classList.toggle("_active");
        }
    }));
    document.addEventListener("selectCallback", (e => {
        e.detail.select.dispatchEvent(new Event("change"));
